...
class ErrorVault_Admin {
    ...
    public function sanitize_settings($input) {
        ...
        // Response hardening is opt-in. These settings affect responses generated
        // by WordPress; host/CDN headers are left alone and verified externally.
        $sanitized['security_header_nosniff'] = $checkbox('security_header_nosniff', false);
        $sanitized['security_header_referrer_policy'] = $checkbox('security_header_referrer_policy', false);
        $sanitized['security_header_frame_options'] = $checkbox('security_header_frame_options', false);
        $sanitized['security_header_permissions_policy'] = $checkbox('security_header_permissions_policy', false);
        $sanitized['security_remove_powered_by'] = $checkbox('security_remove_powered_by', false);
        $policy = isset($input['security_permissions_policy']) ? $input['security_permissions_policy'] : ErrorVault_Security_Hardening::DEFAULT_PERMISSIONS_POLICY;
        $sanitized['security_permissions_policy'] = ErrorVault_Security_Hardening::sanitize_permissions_policy($policy);
    ...
    public function render_settings_page() {
        ...
        <div class="wrap errorvault-settings">
            ...
            <form method="post" action="options.php">
                ...
                <div class="errorvault-card">
                    <h2><?php _e('Security Hardening', 'errorvault'); ?></h2>
                    <p class="description" style="margin-bottom: 15px;">
                        <?php _e('Add selected headers to responses generated by WordPress. Existing headers are preserved. Cached pages, static files and server/CDN responses may bypass these settings, so run a new external scan after saving.', 'errorvault'); ?>
                    </p>

                    <table class="form-table">
                        <tr>
                            <th scope="row"><?php _e('MIME Sniffing', 'errorvault'); ?></th>
                            <td>
                                <label>
                                    <input type="checkbox" name="errorvault_settings[security_header_nosniff]" value="1"
                                        <?php checked(!empty($settings['security_header_nosniff'])); ?>>
                                    <?php _e('Send X-Content-Type-Options: nosniff', 'errorvault'); ?>
                                </label>
                                <p class="description"><?php _e('Prevents browsers from interpreting a response as a different content type.', 'errorvault'); ?></p>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><?php _e('Referrer Policy', 'errorvault'); ?></th>
                            <td>
                                <label>
                                    <input type="checkbox" name="errorvault_settings[security_header_referrer_policy]" value="1"
                                        <?php checked(!empty($settings['security_header_referrer_policy'])); ?>>
                                    <?php _e('Send Referrer-Policy: strict-origin-when-cross-origin', 'errorvault'); ?>
                                </label>
                                <p class="description"><?php _e('Keeps full referrer details on this origin and sends only the origin to HTTPS destinations.', 'errorvault'); ?></p>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><?php _e('Frame Protection', 'errorvault'); ?></th>
                            <td>
                                <label>
                                    <input type="checkbox" name="errorvault_settings[security_header_frame_options]" value="1"
                                        <?php checked(!empty($settings['security_header_frame_options'])); ?>>
                                    <?php _e('Send X-Frame-Options: SAMEORIGIN', 'errorvault'); ?>
                                </label>
                                <p class="description"><?php _e('Reduces clickjacking risk. Leave this off if another website must embed your pages in a frame.', 'errorvault'); ?></p>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><?php _e('Browser Features', 'errorvault'); ?></th>
                            <td>
                                <label>
                                    <input type="checkbox" name="errorvault_settings[security_header_permissions_policy]" value="1"
                                        <?php checked(!empty($settings['security_header_permissions_policy'])); ?>>
                                    <?php _e('Send a Permissions-Policy header', 'errorvault'); ?>
                                </label>
                                <p style="margin-top: 8px;">
                                    <input type="text" name="errorvault_settings[security_permissions_policy]" id="security_permissions_policy"
                                        value="<?php echo esc_attr($settings['security_permissions_policy'] ?? ErrorVault_Security_Hardening::DEFAULT_PERMISSIONS_POLICY); ?>"
                                        class="large-text" maxlength="1000">
                                </p>
                                <p class="description"><?php _e('The default disables sensitive browser features. Adjust it before enabling if the site uses location, camera, microphone, payments or USB.', 'errorvault'); ?></p>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><?php _e('Technology Disclosure', 'errorvault'); ?></th>
                            <td>
                                <label>
                                    <input type="checkbox" name="errorvault_settings[security_remove_powered_by]" value="1"
                                        <?php checked(!empty($settings['security_remove_powered_by'])); ?>>
                                    <?php _e('Remove X-Powered-By from PHP responses', 'errorvault'); ?>
                                </label>
                                <p class="description"><?php _e('Your host or CDN may add it again; disabling expose_php in PHP configuration is more complete.', 'errorvault'); ?></p>
                            </td>
                        </tr>
                    </table>

                    <p class="description">
                        <strong><?php _e('Not managed here:', 'errorvault'); ?></strong>
                        <?php _e('Configure HSTS at the web server or CDN only after every required subdomain supports HTTPS. A Content-Security-Policy needs site-specific testing and should begin in Report-Only mode.', 'errorvault'); ?>
                    </p>
                </div>
